#include "BSpline.h" // Adjust as necessary
#include <cmath> // For std::pow
#include <stdexcept> // For std::out_of_range

BSpline::BSpline( const point_vec &controlPoints )
...
void BSpline::updateControlPoint( int index, const CAGD_POINT &newPoint )
{
...
double BSpline::binomialCoefficient( int n, int k ) const
{
  if( k > n ) return 0;
  double result = 1;
  for( int i = 0; i < k; ++i )
...
double BSpline::bernsteinPolynomial( int i, int n, double t ) const
{
...
void BSpline::computeBasisMatrix() const
{
